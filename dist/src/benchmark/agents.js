...
export function getDriver(harness) {
    const drivers = { codex: codexDriver, "claude-code": claudeDriver, opencode: opencodeDriver };
    return drivers[harness];
}
export async function detectDrivers() {
    const binaries = { codex: "codex", "claude-code": "claude", opencode: "opencode" };
    return Object.fromEntries(await Promise.all(Object.keys(binaries).map(async (name) => {
...
const claudeDriver = {
    ...
    async run({ cwd, prompt, model, endpoint, apiKeyEnv, timeoutMs }) {
        ...
        if (endpoint) {
            ...
            env.ANTHROPIC_AUTH_TOKEN = apiKeyEnv ? process.env[apiKeyEnv] ?? "" : "";
        }
...
function numericModel(model) { return model ? ["--model", model] : []; }
async function scopedCodexHome(endpoint, apiKeyEnv) {
...
async function fetchOpenAiCompatibleModels(endpoint, apiKeyEnv) {
    try {
        ...
        return (body.data ?? []).map((entry) => entry.id).filter((id) => !!id);
    }
    catch {
        return [];
    }
...
export function scanTokenUsage(output) {
    const documents = [];
    ...
    for (const document of documents) {
        ...
        if (outputTokens !== null)
            tokensOut = (tokensOut ?? 0) + outputTokens;
    }
    return { tokensIn, tokensOut };
}
function tryParse(text) { if (!text)
    return null; try {
...
function findUsage(value, depth = 0) {
    if (depth > 4 || typeof value !== "object" || value === null)
        return null;
    const record = value;
    for (const key of ["usage", "token_count"])
