...
const test = (name, body) => `import assert from "node:assert/strict";\nimport test from "node:test";\n${body}`;
...
export async function scaffoldTask(parent, task) {
...
export async function validateTask(parent, task) {
    ...
    try {
        ...
        for (const [path, content] of Object.entries(task.referenceFix))
            await writeFile(join(cwd, path), content);
...
async function git(cwd, args) { const result = await run("git", args, cwd, 30_000, 1_000_000); if (result.exitCode !== 0)
    throw new Error(`git ${args[0]} failed: ${result.stderr}`); }
async function nodeTest(cwd, file) { const result = await run("node", ["--test", file], cwd, 60_000, 4_000_000); return result.exitCode === 0; }
