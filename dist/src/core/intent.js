...
const stripPasteMarkers = (text) => text.replace(/<\/?pasted_content[^>]*>/g, "");
export function extractContract(rawIntent, inspection = {}) {
...
export function detectAmbiguity(contract) {
    const alternatives = [...contract.intent.matchAll(/(?:either|whether)\s+([^\n.]+)/gi)].map((match) => match[1] ?? "");
    if (contract.unknowns.length)
        return { costly: true, question: `What observable behavior should determine completion for: “${contract.goal.slice(0, 120)}”?`, alternatives };
    return { costly: false, alternatives: [] };
}
function materialUnknowns(intent, criteria) {
    const alternatives = [...intent.matchAll(/(?:either|whether)\s+([^\n.]+)/gi)].map((match) => match[1]?.trim()).filter((value) => Boolean(value));
    ...
    if (alternatives.length && material)
        return alternatives;
...
function taskSize(intent, paths, inspection) {
    if (/\b(?:repository-wide|architecture|migration|all packages|runtime policy|systemic)\b/i.test(intent))
        return "systemic";
    const roots = new Set(paths.map((path) => path.split("/")[0]));
    if (roots.size > 1 || /\b(?:cross-package|multiple packages|distributed|integration)\b/i.test(intent))
        return "distributed";
    if (paths.length <= 1 && /\b(?:typo|readme|comment|rename local|one-line)\b/i.test(intent) && (inspection.dependencies?.length ?? 0) >= 0)
        return "tiny";
    return "local";
}
