...
export function deduplicateInstructions(values) {
    ...
    return [...values].sort((a, b) => rank[a.source] - rank[b.source]).filter((item) => {
        ...
        if ([...concepts].some((known) => known.includes(concept) || concept.includes(known)))
            return false;
