...
export function assessScope(contract, changes, beforeDependencies, afterDependencies, index, publicExports = {}) {
    ...
    for (const change of changes) {
        ...
        if (change.added + change.removed > 250 && !hardSignals.some((item) => item.endsWith(change.path)))
            softSignals.push(`large local change: ${change.path}`);
    }
    if (changes.length > 8 && !hardSignals.length)
        softSignals.push(`broad local footprint: ${changes.length} files`);
