import { planActivation } from "./selector.js";
import { unresolved } from "./uncertainty.js";
export function controlRuntime(state, input) {
    ...
    const pressure = { uncertainty: { ...control.uncertainty }, falseActivationCost: input.falseActivationCost ?? 1, missedActivationCost: input.missedActivationCost ?? 1, budgetRemaining: Math.max(0, control.budget.interventions - control.interventionsUsed), context: contextPressure(state) };
    const boundary = boundaryDecision(state, input.trigger, pressure.context);
    const decision = { event, trigger: input.trigger, candidates: plan.trace.candidates, rejected: plan.trace.rejected, ...(plan.trace.selected[0] ? { selected: plan.trace.selected[0] } : {}), pressure, stateChange: input.stateChange ?? [], proofGain: input.proofGain ?? [] };
    ...
    if (plan.trace.selected[0])
        control.hysteresis.lastAction = plan.trace.selected[0];
    else
        delete control.hysteresis.lastAction;
    const directive = boundary.stable && (pressure.context === "rising" || pressure.context === "high") ? { action: "compact", reason: "Stable lifecycle boundary under measured context pressure", artifactRefs: control.context.artifactRefs.slice(-3) } : unresolved(control.uncertainty).length && !plan.trace.selected.length ? { action: "inspect", reason: "Material uncertainty remains without an admitted action" } : { action: "continue", reason: plan.trace.selected.length ? "Selected the cheapest sufficient action" : "No control action is required" };
    return { plan, boundary, decision, directive };
}
export function boundaryDecision(state, trigger, pressure = contextPressure(state)) {
...
function contextPressure(state) {
    const bytes = state.activities.slice(-20).reduce((sum, item) => sum + item.outputBytes, 0);
    if (!state.activities.length)
        return "unknown";
    if (bytes > 500_000)
        return "high";
    if (bytes > 100_000 || state.control.repeatReadsDetected > 4 || state.control.repeatSearchesDetected > 2)
        return "rising";
    return "low";
}
