...
export function conditionOutput(result, limit = 8_000) {
    const raw = [result.stdout, result.stderr].filter(Boolean).join("\n"), lines = raw.split(/\r?\n/), seen = new Set();
    const unique = lines.filter((line) => { const value = line.trim(); if (!value || seen.has(value))
        return false; seen.add(value); return true; });
