...
export function selectVerification(profile, changes, files = [], structural, control) {
    if (changes.length > 0 && changes.every((change) => /(?:\.md|\.txt|\.rst)$/i.test(change.path)))
        return selected({ checks: [{ id: "diff-check", reason: "Documentation-only changes require only a cheap patch integrity check", command: "git", args: ["diff", "--check"] }], rationale: ["Skipped build and behavioral suites for documentation-only changes."] }, control);
    ...
    if (test && changes.length) {
        ...
        if (profile.testRunner && tests.length && !broad && (changedTests.length === changed.length || fullyCovered))
            checks.push({ id: "impacted-tests", reason: `Selected ${tests.length} related test file${tests.length === 1 ? "" : "s"} using the ${profile.testRunner} runner`, command: test.command, args: [...test.args, ...(profile.packageManager === "npm" ? ["--"] : []), ...tests] });
        else
            checks.push({ id: "repository-tests", reason: "Changed files require behavioral regression proof; no safe impacted-test target was identified", command: test.command, args: test.args });
    }
...
export function verificationCandidates(plan) {
...
function selected(plan, control) {
    if (!control || !plan.checks.length)
        return plan;
