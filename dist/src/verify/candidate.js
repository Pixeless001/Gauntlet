...
export function evaluateCandidate(world, node, evaluation) {
    const candidate = node.candidate;
    if (!candidate)
        return { disposition: "rejected", reasons: ["missing candidate result"] };
    const checks = [
    ...
    if (reasons.includes("candidate input world is stale"))
        return { disposition: "stale", reasons };
    if (reasons.length)
        return { disposition: "rejected", reasons };
    if (candidate.unresolved.length || evaluation.semanticAmbiguity)
        return { disposition: "semantic-verification-required", reasons: candidate.unresolved.map((item) => `unresolved ${item}`) };
    return { disposition: "validated", reasons: [] };
}
export function applyCandidateEvaluation(world, nodeId, evaluation) {
    const node = world.work.nodes[nodeId];
    if (!node)
        throw new Error(`Unknown work node: ${nodeId}`);
    if (evaluation.disposition === "validated") {
        const fact = { id: `work:${nodeId}`, provenance: nodeId, statement: `${node.title} validated`, evidenceRefs: [...node.evidenceRefs], fingerprint: world.fingerprint.value, version: (world.facts[`work:${nodeId}`]?.version ?? 0) + 1, status: "validated" };
    ...
    if (evaluation.disposition === "semantic-verification-required")
        return world;
