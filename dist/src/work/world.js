...
const normalizeInputs = (inputs) => ({ ...inputs, config: inputs.config ?? {}, upstream: inputs.upstream ?? {} });
export function createWorld(contract, canonicalRevision, inputs) {
...
export function refreshWorld(world, inputs, canonicalRevision) {
    const normalized = normalizeInputs(inputs), value = fingerprint(normalized);
    if (value === world.fingerprint.value && canonicalRevision === world.canonicalRevision)
        return world;
    return invalidateDecision({ ...world, revision: world.revision + 1, canonicalRevision, fingerprint: { ...normalized, value } });
}
export function refreshValidityInputs(world, inputs, canonicalRevision) {
    const normalized = normalizeInputs(inputs), sources = changedSources(world.fingerprint, normalized, world.canonicalRevision, canonicalRevision);
    let next = world;
    for (const source of sources)
        next = invalidateCone(next, source).world;
    return refreshWorld(next, normalized, canonicalRevision);
}
export function refreshCapabilities(world, capabilities) {
    const next = [...new Set(capabilities)].sort();
    if (JSON.stringify(next) === JSON.stringify(world.capabilities))
        return world;
...
export function refreshRules(world, rules) {
    const next = [...new Set(rules)].sort();
    if (JSON.stringify(next) === JSON.stringify(world.rules))
        return world;
...
function changedSources(current, next, currentRevision, nextRevision) {
    const sources = [];
    ...
    if (currentRevision !== nextRevision)
        sources.push("revision");
    for (const path of new Set([...Object.keys(current.files), ...Object.keys(next.files)]))
        if (current.files[path] !== next.files[path])
            sources.push(`file:${path}`);
    for (const name of new Set([...Object.keys(current.packages), ...Object.keys(next.packages)]))
        if (current.packages[name] !== next.packages[name])
            sources.push(`package:${name}`);
    for (const path of new Set([...Object.keys(current.config), ...Object.keys(next.config)]))
        if (current.config[path] !== next.config[path])
            sources.push(`config:${path}`);
    for (const id of new Set([...Object.keys(current.upstream), ...Object.keys(next.upstream)]))
        if (current.upstream[id] !== next.upstream[id])
            sources.push(`upstream:${id}`);
    return sources;
}
