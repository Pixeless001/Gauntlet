...
export class GraphEventStore {
    root;
    constructor(cwd) { this.root = resolve(cwd, ".gauntlet", "sessions"); }
    async append(taskId, event, world) {
    ...
    async read(taskId, after = 0) {
        try {
            const lines = (await readFile(join(this.directory(taskId), "events.jsonl"), "utf8")).split("\n").filter(Boolean);
            const events = lines.map((line) => JSON.parse(line));
            if (events.some((event, index) => !Number.isInteger(event.sequence) || event.sequence !== index + 1))
                throw new Error("Invalid graph event sequence");
    ...
    async resume(taskId) {
        ...
        let checkpoint = null;
        try {
            checkpoint = JSON.parse(await readFile(join(this.directory(taskId), "world.json"), "utf8"));
        }
        catch (error) {
            if (error.code !== "ENOENT")
                throw error;
        ...
        for (const event of events.filter((item) => item.sequence > sequence)) {
            if (event.world)
                ...
                world = applyPatch(world, event.patch);
            else
                throw new Error(`Graph event ${event.sequence} has no replayable state`);
...
function visit(before, after, path, patches) {
    ...
    if (isRecord(before) && isRecord(after)) {
        for (const key of new Set([...Object.keys(before), ...Object.keys(after)])) {
            if (!Object.hasOwn(after, key))
                ...
                patches.push({ path: [...path, key], value: after[key] });
            else
                visit(before[key], after[key], [...path, key], patches);
        }
        return;
    }
    patches.push({ path, value: after });
}
function applyPatch(before, patches) {
    let result = before === undefined ? undefined : structuredClone(before);
    for (const patch of patches) {
        if (!patch.path.length) {
            if (patch.remove)
                throw new Error("Graph replay cannot remove its root");
            result = structuredClone(patch.value);
            continue;
        }
        if (!isRecord(result))
            throw new Error("Graph replay has no base world");
        let target = result;
    ...
    if (!isRecord(result))
        throw new Error("Graph replay produced no world");
    return result;
}
...
async function withLock(lock, run) {
    for (let attempt = 0; attempt < 250; attempt++) {
        ...
        if (attempt === 249)
            throw new Error("Timed out acquiring graph event lock");
    }
    try {
        return await run();
    }
