...
export function selectInstructionSections(content, intent, limit = 2_000) {
    ...
    for (const { text, index } of ranked) {
        ...
        if (room < 120)
            break;
        const part = (text.length <= room ? text : rulesOnly(text)).slice(0, room);
        chosen.set(index, part);
        used += part.length + 2;
    }
