...
export async function runCapabilityBoundary(input) {
    const request = redact(input.request), count = active.get(input.source) ?? 0, limit = input.maxConcurrency ?? 2;
    if (count >= limit)
        return { status: "unavailable", source: input.source, request, reason: "concurrency limit reached" };
    const controller = new AbortController(), timeout = setTimeout(() => controller.abort(), input.timeoutMs ?? 10_000);
    active.set(input.source, count + 1);
    try {
        const result = await input.run(request, controller.signal);
        return { status: "ok", source: input.source, request, result: typeof result === "string" ? redact(result) : result };
    }
    catch (error) {
        ...
        const remaining = (active.get(input.source) ?? 1) - 1;
        if (remaining)
            active.set(input.source, remaining);
        else
            active.delete(input.source);
    }
}
