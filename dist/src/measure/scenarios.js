...
import { activePath, rejectBranch } from "../execution-state/checkpoints.js";
...
import { createControlState, createTaskWorld } from "../core/task-state.js";
import { boundaryDecision } from "../control/runtime.js";
export function runIntelligenceEvals() {
    ...
    const marginal = selectMarginal([{ value: "owner", contributions: ["owner:session"], cost: 1 }, { value: "duplicate", contributions: ["owner:session"], cost: 1 }, { value: "test", contributions: ["acceptance:race"], cost: 1 }], 2);
    const state = task("Implement responsive behavior in ui.tsx"), visual = requiredCapabilities(state);
    state.control.context.pressure = "unknown";
...
function point(id, kind, status) { return { id, kind, status, summary: id, constraints: [], decisions: [], relevantFiles: [], relevantSymbols: [], proofRefs: [], createdFromEvent: 0, resolves: [] }; }
function result(caseId, category, passed, started, interventions, proof) { return { category, caseId, passed, durationMs: performance.now() - started, interventions, extraModelCalls: 0, contextItems: 0, repeatedReads: 0, rawOutputBytes: 0, conditionedOutputBytes: 0, proof }; }
