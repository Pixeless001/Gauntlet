...
export const ARCHITECTURE_AUDIT = [
    ...
    done("sparse-context", "sparse worker and cold verification views", "Workers and verifiers receive only communication-graph-authorized facts and evidence; conflicts create a bounded synthesis request.", ["src/evidence/packets.ts#compileWorkerPacket", "src/evidence/packets.ts#createVerificationView", "src/work/reducer.ts#semanticSynthesisRequest", "src/core/engine.ts#async finish"], ["test/engine.test.ts#one worker receives only its sparse packet", "test/work-reduction.test.ts#candidate reducer preserves independent evidence"]),
    ...
    done("adapters", "host capability boundaries", "Adapters declare result replacement, cancellation, worktree, delegation, and compaction truthfully.", ["src/adapters/types.ts#HarnessCapabilities", "src/hooks/dispatch.ts#activityOutput"], ["test/hooks.test.ts#host adapters condition results only through declared replacement paths", "test/adapters.test.ts#adapters publish individually testable capabilities"]),
    ...
    done("evaluation", "executable evaluation", "Decision, repository, replay, ablation, interaction, orchestration, and substrate suites execute structured cases.", ["src/measure/eval-runner.ts#runEvalSuite", "src/measure/eval-runner.ts#runSubstrateSuite"], ["test/evals.test.ts#every evaluation suite executes structured cases", "test/evals.test.ts#research-derived orchestration suites remain executable"]),
    done("substrate", "native substrate conformance", "The shipping engine has no LangGraph dependency, exposes only the narrow execution boundary, and is selected after a repeatable measured external probe (npm run substrate:probe).", ["src/execution/native-engine.ts#NativeExecutionEngine", "src/measure/substrate.ts#LANGGRAPH_PROBE", "src/measure/substrate.ts#SUBSTRATE_MEASUREMENTS"], ["test/native-engine.test.ts#native engine checkpoints", "test/evals.test.ts#substrate comparison covers"]),
    done("metrics", "separate runtime-loop metrics", "Outcome, control/context, evidence, and orchestration counters separately record critical path, frontier delay/width, fan-out, evidence duplication, retries, conflicts, workers, synthesis, and verifier yield.", ["src/core/measure.ts#frontierWait", "src/core/measure.ts#measure", "src/core/engine.ts#async finish"], ["test/core.test.ts#measurement separates frontier, evidence, and redo metrics"]),
    ...
    done("benchmark", "paired agent benchmark", "Real agent CLIs run a counterfactually validated corpus with and without Gauntlet hooks in isolated workspaces; acceptance, preservation, slop findings, wall time, tokens, and LoC are reported per arm.", ["src/benchmark/runner.ts#runBenchmark", "src/benchmark/fixtures.ts#benchmarkTasks", "src/benchmark/agents.ts#scanTokenUsage"], ["test/benchmark.test.ts#benchmark corpus tasks are counterfactually valid", "test/benchmark.test.ts#report pairs arms and computes rates, medians, and slop codes"]),
];
export function summarizeArchitectureAudit(items = ARCHITECTURE_AUDIT) {
    const ids = items.map((item) => item.id);
    if (new Set(ids).size !== ids.length)
        throw new Error("Architecture audit contains duplicate capability ids");
...
export async function validateAuditProof(cwd, items = ARCHITECTURE_AUDIT) {
    const root = resolve(cwd), canonicalRoot = await realpath(root), issues = [];
    for (const item of items) {
        if (!item.implementation.length || !item.tests.length) {
            issues.push({ id: item.id, kind: item.implementation.length ? "test" : "implementation", proof: "", reason: "missing" });
            continue;
        }
        for (const [kind, entries] of [["implementation", item.implementation], ["test", item.tests]])
            for (const proof of entries) {
                ...
                try {
                    ...
                    if (anchor && !(await readFile(path, "utf8")).includes(anchor))
                        issues.push({ id: item.id, kind, proof, reason: "unverified" });
                }
                catch {
                    issues.push({ id: item.id, kind, proof, reason: "missing" });
                }
...
function done(id, capability, acceptance, implementation, tests) { return { id, capability, acceptance, mandatory: true, status: "implemented", implementation, tests }; }
function partial(id, capability, acceptance, implementation, tests, gap) { return { id, capability, acceptance, mandatory: true, status: "partial", implementation, tests, ...(gap ? { gap } : {}) }; }
