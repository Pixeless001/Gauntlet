...
test("packaged skills have unique valid frontmatter", async () => {
  ...
  for (const directory of directories) {
    ...
    assert.match(fields.name ?? "", /^[a-z0-9-]+$/); assert.equal(fields.name, directory); assert.ok(fields.description); assert.ok(fields.budget); assert.equal(names.has(fields.name!), false); names.add(fields.name!);
  }
});
