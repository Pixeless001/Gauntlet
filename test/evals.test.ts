...
import { LANGGRAPH_PROBE, SUBSTRATE_COMPARISON, SUBSTRATE_MEASUREMENTS } from "../src/measure/substrate.js";
...
test("substrate comparison covers the non-shipping LangGraph probe and native selection", () => {
  ...
  assert.equal(SUBSTRATE_MEASUREMENTS.langgraph.checkpointReadbackOk, true); assert.ok(SUBSTRATE_MEASUREMENTS.langgraph.importMs > SUBSTRATE_MEASUREMENTS.native.importMs); assert.ok(SUBSTRATE_MEASUREMENTS.langgraph.dependencyDirs > SUBSTRATE_MEASUREMENTS.native.dependencyDirs);
});
