...
test("architecture audit exposes incomplete mandatory architecture", async () => {
  ...
  assert.ok(summary.total >= 20); assert.equal(summary.mandatory, summary.total); assert.equal(summary.releaseReady, false); assert.ok(summary.blocking.includes("sparse-context")); assert.equal(summary.blocking.includes("candidates"), false); assert.equal(summary.blocking.includes("event-sourcing"), false); assert.deepEqual(issues, []);
