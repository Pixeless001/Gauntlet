...
export async function runVerification(cwd: string, plan: VerificationPlan, environment: ExecutionEnvironment = new LocalExecutionEnvironment(cwd), runId = "verification"): Promise<VerificationResult[]> {
  ...
  for (const check of plan.checks) {
    ...
    const conditioned = conditionOutput(result), evidence = await storeOutput(cwd, result, runId);
    results.push({ ...result, id: check.id, reason: check.reason, status, summary: conditioned.summary, evidence, conditionedBytes: conditioned.retainedBytes, tokensRemoved: conditioned.tokensRemoved, actionableFailures: conditioned.actionableFailures, outputTruncated: conditioned.truncated });
