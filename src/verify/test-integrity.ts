...
export async function inspectTestIntegrity(cwd: string, before: Record<string, TestSignature>, changes?: FileDelta[]): Promise<Finding[]> {
  ...
  for (const [path, signature] of Object.entries(before)) {
    ...
    if (!after[path]) { findings.push({ code: "test-deleted", severity: "error", blocking: true, message: "A test file was deleted during the task.", proof: [path] }); continue; }
    ...
    if (missing.length) findings.push({ code: weakening ? "assertion-weakened" : "assertion-changed", severity: "warning", blocking: weakening, message: weakening ? "An existing assertion was removed or replaced by broader proof." : "Existing assertions changed; review the task-scoped behavioral update.", proof: [path, ...missing.slice(0, 3)] });
    if (after[path]!.skipped > signature.skipped) findings.push({ code: "tests-skipped", severity: "error", blocking: true, message: "The number of skipped tests increased.", proof: [path, `${signature.skipped} → ${after[path]!.skipped}`] });
  }
  for (const path of changed ?? []) if (!(path in before) && after[path]?.skipped) findings.push({ code: "tests-skipped", severity: "error", blocking: true, message: "A new test was committed skipped or pending.", proof: [path, `0 → ${after[path]!.skipped}`] });
  return findings;
}
