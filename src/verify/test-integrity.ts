...
export async function inspectTestIntegrity(cwd: string, before: Record<string, TestSignature>, changes?: FileDelta[]): Promise<Finding[]> {
  ...
  for (const [path, signature] of Object.entries(before)) {
    ...
    if (after[path]!.skipped > signature.skipped) findings.push({ code: "tests-skipped", severity: "error", message: "The number of skipped tests increased.", evidence: [path, `${signature.skipped} → ${after[path]!.skipped}`] });
  }
  return findings;
}
