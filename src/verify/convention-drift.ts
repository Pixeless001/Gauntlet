...
export async function inspectConventionDrift(cwd: string, state: TaskState, changes: FileDelta[]): Promise<Finding[]> {
  ...
  for (const name of added) {
    ...
    if (capability && established) findings.push({ code: "convention-dependency-conflict", severity: "warning", blocking: false, message: `New ${capability} dependency ${name} conflicts with the established ${established.value} convention.`, evidence: [name, ...established.representatives.slice(0, 1)] });
  }
  for (const change of changes) {
    const placement = strong.find((fact) => testPlacementConflict(fact, change.path));
    if (placement && change.added > 0) findings.push({ code: "convention-test-placement", severity: "warning", blocking: false, message: "New test placement conflicts with the repository's colocated-test convention.", evidence: [change.path] });
