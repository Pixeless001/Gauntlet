...
export interface SlopSignal { code: string; message: string; proof: string[] }

export function analyzeSlopLines(path: string, lines: string[]): SlopSignal[] {
  if (!CODE_FILE.test(path)) return [];
  const signals: SlopSignal[] = [], code = lines.filter((line) => !/^\s*(?:\/\/|\/\*|\*)/.test(line));
  const commentLines = lines.length - code.length;
  if (commentLines >= 5 && lines.length >= 10 && commentLines / lines.length >= 0.4) signals.push({ code: "slop-comment-density", message: `Heavy comment density in added lines (${commentLines}/${lines.length}).`, proof: [path] });
  lines.forEach((line, index) => {
    if (NARRATIONAL.test(line) || AI_PHRASING.test(line)) { signals.push({ code: "slop-narrational-comment", message: "Added comment narrates the implementation instead of stating a constraint.", proof: [`${path}:${index + 1}`, line.trim().slice(0, 120)] }); return; }
    if (DECORATED_LOG.test(line)) signals.push({ code: "slop-decorated-log", message: "Added console logging decorated with emoji/visual noise.", proof: [`${path}:${index + 1}`] });
  });
...
export async function inspectSlopForDiff(cwd: string, base: string, changes: FileDelta[]): Promise<Finding[]> {
  ...
  for (const [path, lines] of Object.entries(addedByFile)) {
    ...
    findings.push(...analyzeSlopLines(path, lines).map((signal) => ({ code: signal.code, severity: "warning" as const, blocking: false, message: signal.message, proof: signal.proof })));
...
async function deadExports(cwd: string, path: string, lines: string[]): Promise<Finding[]> {
  const names = [...lines.join("\n").matchAll(EXPORTED)].map((match) => match[1]).filter((name): name is string => !!name);
  const found: Finding[] = [];
  for (const name of names) {
    const grep = await run("git", ["grep", "-l", "-F", name, "--", "*.ts", "*.js", "*.tsx", "*.jsx"], cwd, 15_000, 1_000_000);
    const files = grep.stdout.trim().split("\n").filter(Boolean);
    if (files.length === 1 && files[0] === path) found.push({ code: "slop-dead-export", severity: "warning", blocking: false, message: `Newly exported symbol ${name} has no references outside its defining file.`, proof: [path, name] });
  }
  return found;
}

async function complexitySpike(cwd: string, base: string, path: string): Promise<Finding[]> {
  if (!CODE_FILE.test(path)) return [];
  let before = "";
  try { before = (await run("git", ["show", `${base}:${path}`], cwd, 15_000, 4_000_000)).stdout; } catch { return []; }
  const after = await (async () => { try { return await readFile(`${cwd}/${path}`, "utf8"); } catch { return ""; } })();
  if (!after) return [];
  const count = (text: string) => (text.match(BRANCHES) ?? []).length;
  const [b, a] = [count(before), count(after)];
  if (b >= 8 && a >= b * 2 && a - b >= 15) return [{ code: "slop-complexity-spike", severity: "warning", blocking: false, message: `Branch-keyword count in ${path} grew from ${b} to ${a}.`, proof: [path, `${b}->${a}`] }];
  return [];
}
