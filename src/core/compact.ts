...
export interface ContinuationRecord { task: string; goal: string; acceptanceCriteria: string[]; preservationRequirements: string[]; constraints: string[]; repoConstraints: string[]; workingSet: string[]; currentApproach: string; unresolved: string[]; failedApproaches: string[]; artifactRefs: string[]; recentTurns: { target?: string; outcome?: "pass" | "fail" | "unknown"; artifactRef?: string }[]; workflow?: string; guidance?: string }
...
export function compact(state: TaskState): ContinuationRecord {
  ...
  return {
    ...
    failedApproaches: state.control?.execution ? (active.rejectedWarning ? [active.rejectedWarning] : []) : state.control?.failedApproaches.length ? [...state.control.failedApproaches] : [...new Set(state.activities.filter((item) => item.outcome === "fail").map((item) => item.target).filter((x): x is string => Boolean(x)))],
