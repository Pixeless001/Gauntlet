...
export interface OrchestrationTrace { frontierWaitMs: number; maxFrontier: number; semanticSynthesis: number; workerCandidates: number }
...
export interface TaskMeasurement {
  ...
  orchestration?: { nodes: number; requiredNodes: number; validatedNodes: number; staleNodes: number; collapsedNodes: number; criticalPath: number; decisionRevision: number; eventSequence: number; totalNodeAttempts: number; duplicateEvidence: number; frontierWaitMs: number; maxFrontier: number; fanOutBenefit: number; workerCandidates: number; semanticSynthesis: number; verifierYield: number; redoRate: number; writeConflicts: number; };
}

export function measure(state: TaskState, changes: FileDelta[], results: VerificationResult[], now = new Date(), completion?: CompletionDecision, trace: OrchestrationTrace = { frontierWaitMs: 0, maxFrontier: 0, semanticSynthesis: 0, workerCandidates: 0 }): TaskMeasurement {
...
export function frontierWait(events: readonly Pick<GraphEvent, "at" | "type" | "nodeId">[]): number {
  const ready = new Map<string, number>(); let total = 0;
  for (const event of events) {
    if (!event.nodeId) continue;
    if (event.type === "NODE_READY") ready.set(event.nodeId, Date.parse(event.at));
    if (event.type === "NODE_STARTED") { const at = ready.get(event.nodeId); if (at !== undefined) total += Math.max(0, Date.parse(event.at) - at); ready.delete(event.nodeId); }
  }
  return total;
}
