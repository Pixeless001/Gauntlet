...
export async function evaluateGuards(cwd: string, state: TaskState, changes: FileDelta[]): Promise<Finding[]> {
  ...
  if (changes.length > 8 && total > 300) findings.push({ code: "scope-growth", severity: "warning", blocking: false, message: "The implementation footprint is unusually large; confirm every changed file is task-scoped.", evidence: [`${changes.length} files`, `${total} changed lines`] });
  ...
  if (added.length) findings.push({ code: "dependency-added", severity: "warning", blocking: false, message: "New runtime dependencies require justification.", evidence: added });
...
export function loopFinding(state: TaskState): Finding | null {
  ...
  return target ? { code: "repair-loop", severity: "warning", blocking: true, message: "The same failure occurred at least three times; reassess the underlying assumption.", evidence: [target] } : null;
}
