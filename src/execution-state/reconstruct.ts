...
export function reconstruct(state: TaskState): ActiveExecutionContext {
  ...
  if (!execution) return { task: state.contract.intent, acceptanceCriteria: state.contract.acceptanceCriteria, constraints: state.contract.constraints, validatedState: [], current: state.control?.currentApproach ?? "", relevantFiles: state.workingSet, relevantSymbols: [], proofRefs: [], open: state.control?.unresolvedIssues ?? [] };
