...
export async function selectContext(cwd: string, contract: TaskContract, limit = 12, conventions: ConventionFact[] = [], index?: RepoIndex): Promise<ContextPacket> {
  ...
  const scored = files.map((path): ContextEntry => {
    ...
    score += related.get(path)?.score ?? 0; score += terms.filter((term) => lower.includes(term)).length * 3;
    ...
    return { path, score, reason: contract.explicitPaths.some((item) => lower.includes(item.replaceAll("*", "").toLowerCase())) ? "explicit task scope" : related.get(path)?.reason ?? (/test|spec/.test(lower) ? "related test candidate" : "task term match") };
