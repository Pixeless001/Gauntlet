...
export interface CommitRule { source: string; text: string; terms: string[] }
...
export function commitRules(source: string, content: string): CommitRule[] {
...
export async function inspectCommitRules(cwd: string, head: string | null): Promise<Finding[]> {
  ...
  const commits = log.stdout.split("\x01").map((entry) => entry.trim().split("\0")).filter(([sha]) => sha) as [string, string?][];
  return rules.flatMap((rule) => {
    const violated = commits.filter(([, message = ""]) => rule.terms.some((term) => new RegExp(`^\\W*${escape(term)}`, "im").test(message))).map(([sha]) => sha);
    return violated.length ? [{ code: "convention-instruction", severity: "warning" as const, blocking: false, message: `${rule.source}: "${rule.text.slice(0, 160)}" — not followed in ${violated.join(", ")}`, proof: violated }] : [];
  });
}
