...
export class ConventionCache {
  ...
  async load(): Promise<RepoConventionProfile> {
    try {
      ...
      const profile = { version: 1 as const, facts };
      if (JSON.stringify(raw) !== JSON.stringify(migrated)) await this.save(profile);
      return profile;
...
async function packageFacts(cwd: string, files: string[], budget: ConventionBudget): Promise<ConventionFact[]> {
  try {
    ...
    if (pkg.scripts?.lint) facts.push({ id: "tool.linter", category: "tooling", value: pkg.scripts.lint, strength: "strong", scope: ".", sourceRefs: src, representatives: [] });
    if (pkg.scripts?.format) facts.push({ id: "tool.formatter", category: "tooling", value: pkg.scripts.format, strength: "strong", scope: ".", sourceRefs: src, representatives: [] });
    if (pkg.type === "module") facts.push({ id: "api.module", category: "api", value: "esm", strength: "strong", scope: ".", sourceRefs: src, representatives: [] });
    return facts;
  } catch { return []; }
}
...
async function configFacts(cwd: string): Promise<ConventionFact[]> {
  ...
  for (const [path, id, value] of [["tsconfig.json", "tool.language", "typescript"], ["pyproject.toml", "tool.language", "python"], ["Cargo.toml", "tool.language", "rust"]] as const) {
    try { await stat(join(cwd, path)); facts.push({ id, category: "tooling", value, strength: "strong", scope: ".", sourceRefs: [await source(cwd, path)], representatives: [] }); } catch { /* absent config */ }
  }
  for (const [path, value] of [["package-lock.json", "npm"], ["pnpm-lock.yaml", "pnpm"], ["yarn.lock", "yarn"], ["Cargo.lock", "cargo"], ["uv.lock", "uv"], ["poetry.lock", "poetry"]] as const) {
    try { await stat(join(cwd, path)); facts.push({ id: "tool.package-manager", category: "tooling", value, strength: "strong", scope: ".", sourceRefs: [await source(cwd, path)], representatives: [] }); break; } catch { /* try next lockfile */ }
  }
  return facts;
}
...
export async function discoverConventions(cwd: string, touched: string[] = [], budget = DEFAULT_CONVENTION_BUDGET, index?: RepoIndex): Promise<RepoConventionProfile> {
  ...
  if (routes.length >= 2 && repositories.length >= 2 && services.length >= 2) discovered.push({ id: "architecture.db-access", category: "architecture", value: "routes → services → repositories", strength: "strong", scope: ".", sourceRefs: await Promise.all([...routes.slice(0, 2), ...services.slice(0, 2), ...repositories.slice(0, 2)].map((path) => source(cwd, path))), representatives: [routes[0]!, services[0]!, repositories[0]!] });
