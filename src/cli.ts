...
import { HANDOFF_AUDIT, summarizeHandoffAudit, validateAuditEvidence } from "./measure/handoff-audit.js";
...
async function main() {
  ...
  else if (command === "audit") { const summary = summarizeHandoffAudit(), evidenceIssues = await validateAuditEvidence(cwd); console.log(JSON.stringify({ summary, evidenceIssues, items: HANDOFF_AUDIT }, null, 2)); if (args.includes("--strict") && (!summary.releaseReady || evidenceIssues.length)) process.exitCode = 1; }
