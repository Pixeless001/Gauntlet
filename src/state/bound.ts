...
export function boundState(state: TaskState): TaskState {
  ...
  for (const tier of TIERS) { limit(state, tier); if (persistedSize(state) <= BUDGET) break; }
  return state;
}

// The repo-sized baseline (files, tests, index) is stored in a sidecar, so it does not count against the task file.
const persistedSize = (state: TaskState) => { const { files: _files, tests: _tests, index: _index, ...baseline } = state.baseline; return JSON.stringify({ ...state, baseline }).length; };

function limit(state: TaskState, tier: (typeof TIERS)[number]) {
  ...
  const newest = (prefix: string, count: number) => new Set(Object.keys(state.world.facts).filter((key) => key.startsWith(prefix)).slice(-count));
  ...
  if (Object.keys(facts).length !== Object.keys(state.world.facts).length) state.world = { ...state.world, facts };
  state.findings = state.findings.slice(-tier.findings);
  state.control.decisions = state.control.decisions.slice(-tier.decisions); state.control.traces = state.control.traces.slice(-tier.decisions);
