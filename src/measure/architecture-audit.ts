...
export const ARCHITECTURE_AUDIT: readonly ArchitectureAuditItem[] = [
  ...
  partial("event-sourcing", "durable world events", "Replayable JSONL patches are sequenced before an atomic compact-world checkpoint advances.", ["src/work/event-store.ts#class GraphEventStore", "src/state/store.ts#updateTaskWithWorldEvent"], ["test/world-runtime.test.ts#graph event writes serialize", "test/event-recovery.test.ts#event history stores replay patches"], "Cancellation and every graph mutation are not yet emitted through the same transition path."),
  ...
  partial("verification", "risk-based verification and stopping", "Completion requires preservation, resolved material uncertainty, relevant checks, current repository revision, and readable hashed evidence.", ["src/verify/completion.ts#decideCompletion", "src/core/engine.ts#async finish"], ["test/completion.test.ts#completion rejects repository drift and invalid evidence", "test/engine.test.ts#static checks do not claim behavioral proof"], "Cold semantic judgment is not yet independently invoked."),
