...
export const ARCHITECTURE_AUDIT: readonly ArchitectureAuditItem[] = [
  ...
  done("event-sourcing", "durable world events", "Every graph transition is sequenced in JSONL before its checkpointed world snapshot advances.", ["src/work/event-store.ts#class GraphEventStore", "src/state/store.ts#updateTaskWithWorldEvent"], ["test/world-runtime.test.ts#graph event writes serialize", "test/event-recovery.test.ts#state recovery resumes"]),
  done("world", "current valid world", "The contract-scoped world carries explicit fingerprints, facts, graph revisions, ownership, capabilities, and evidence references.", ["src/work/types.ts#CurrentValidWorld", "src/work/world.ts#refreshWorld"], ["test/world-runtime.test.ts#contract-scoped writes stale"]),
  done("graphs", "work validity communication graphs", "Work readiness, local validity-cone staleness, and permitted evidence flow are native graph semantics.", ["src/work/types.ts#WorkGraph", "src/work/graph.ts#invalidateCone"], ["test/work-graph.test.ts#validity invalidation stales", "test/world-runtime.test.ts#contract-scoped writes stale"]),
  ...
  done("candidates", "candidate-only execution and promotion", "Executor output remains a candidate until ordered deterministic checks promote it into validated facts.", ["src/work/types.ts#CandidateResult", "src/verify/candidate.ts#evaluateCandidate"], ["test/candidate-evaluation.test.ts#deterministic evaluation rejects", "test/work-graph.test.ts#candidate output never validates"]),
  done("scheduler", "native frontier scheduling", "Native scheduling bounds local width, delegates at most one worker, and serializes unisolated mutations.", ["src/work/scheduler.ts#scheduleReady", "src/execution/native-engine.ts#class NativeExecutionEngine"], ["test/native-engine.test.ts#native engine batches", "test/native-engine.test.ts#native engine permits"]),
  done("sparse-context", "sparse worker and cold verification views", "Workers and verifiers receive only communication-graph-authorized facts and evidence.", ["src/evidence/packets.ts#compileWorkerPacket", "src/evidence/packets.ts#createVerificationView"], ["test/candidate-evaluation.test.ts#worker and verifier packets"]),
  ...
  done("evaluation", "executable evaluation", "Decision, repository, replay, ablation, interaction, orchestration, and substrate suites execute structured cases.", ["src/measure/eval-runner.ts#runEvalSuite", "src/measure/eval-runner.ts#runSubstrateSuite"], ["test/evals.test.ts#every evaluation suite executes structured cases"]),
  done("substrate", "native substrate conformance", "The shipping engine has no LangGraph dependency and exposes only the narrow execution boundary.", ["src/execution/native-engine.ts#NativeExecutionEngine", "src/work/types.ts#ExecutionEngine"], ["test/native-engine.test.ts#native engine checkpoints", "test/evals.test.ts#every evaluation suite executes structured cases"]),
  done("metrics", "separate runtime-loop metrics", "Outcome, control/context, evidence, and orchestration counters remain independently inspectable.", ["src/core/measure.ts#orchestration", "src/core/measure.ts#outcome"], ["test/core.test.ts#measurement exposes selection"]),
