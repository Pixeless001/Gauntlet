...
export const ARCHITECTURE_AUDIT: readonly ArchitectureAuditItem[] = [
  partial("state", "validated runtime state", "State loads V1→V2→V3, saves atomically, and excludes raw tool payloads.", ["src/state/store.ts#class StateStore", "src/state/v2-migration.ts#migrateTaskV2"], ["test/state-migration.test.ts#first-version stored outputs become artifact handles", "test/core.test.ts#task state never persists raw tool payloads"], "Stored sessions are not exercised through every adapter lifecycle."),
  partial("event-sourcing", "durable world events", "Every graph transition is sequenced in JSONL before its checkpointed world snapshot advances.", ["src/work/event-store.ts#class GraphEventStore", "src/state/store.ts#updateTaskWithWorldEvent"], ["test/world-runtime.test.ts#graph event writes serialize", "test/event-recovery.test.ts#state recovery replays the last persisted event"], "The event stream has recovery materialization but not a reducer for every transition."),
  partial("world", "current valid world", "The contract-scoped world carries explicit fingerprints, facts, graph revisions, ownership, capabilities, and evidence references.", ["src/work/types.ts#CurrentValidWorld", "src/work/world.ts#refreshWorld"], ["test/world-runtime.test.ts#contract-scoped writes stale"]),
  partial("graphs", "work validity communication graphs", "Work readiness, local validity-cone staleness, and permitted evidence flow are native graph semantics.", ["src/work/types.ts#WorkGraph", "src/work/graph.ts#invalidateCone"], ["test/work-graph.test.ts#validity invalidation stales", "test/work-graph.test.ts#ready frontier excludes stale facts and overlapping ownership"]),
  ...
  partial("verification", "risk-based verification and stopping", "Completion requires preservation, resolved material uncertainty, and relevant checks.", ["src/verify/completion.ts#decideCompletion", "src/core/engine.ts#async finish"], ["test/core.test.ts#measurement never reports success", "test/engine.test.ts#static checks do not claim behavioral proof"], "World/repository/evidence revision consistency is not a completion invariant."),
  partial("candidates", "candidate-only execution and promotion", "Executor output remains a candidate until ordered deterministic checks promote it into validated facts.", ["src/work/types.ts#CandidateResult", "src/verify/candidate.ts#evaluateCandidate"], ["test/candidate-evaluation.test.ts#deterministic evaluation rejects", "test/work-graph.test.ts#candidate output never validates"], "Mutation promotion does not yet apply an isolated patch transaction."),
  partial("scheduler", "native frontier scheduling", "Native scheduling bounds local width, delegates at most one worker, and serializes unisolated mutations.", ["src/work/scheduler.ts#scheduleReady", "src/execution/native-engine.ts#class NativeExecutionEngine"], ["test/native-engine.test.ts#native engine batches", "test/native-engine.test.ts#native engine permits"], "The native engine is wired for candidate execution but the runtime has no broad ready-frontier executor."),
