...
function taskId(input: NativeEvent): string {
  ...
  const identity = candidate ?? `${input.cwd ?? process.cwd()}:${process.ppid}`;
  return `native-${createHash("sha256").update(String(identity)).digest("hex").slice(0, 24)}`;
}
...
export function translateNativeEvent(input: unknown, nativeEvents: string[], explicitName?: string): GauntletEvent {
  ...
  const base = { version: 1 as const, taskId: taskId(value), repository: typeof value.cwd === "string" ? value.cwd : process.cwd(), timestamp: new Date().toISOString() };
