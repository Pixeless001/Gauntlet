...
export function proposeAtomicChange(patterns: ExperiencePattern[]): BehaviorProposal | null {
  const candidate = patterns.filter((item) => item.status === "supported" && item.supportingEvidence.length > item.contradictingEvidence.length).sort((a, b) => b.supportingEvidence.length - a.supportingEvidence.length || a.id.localeCompare(b.id))[0];
...
export function evaluateProposal(pattern: ExperiencePattern, proposal: BehaviorProposal, results: EvalComparison[]): EvolutionDecision {
  if (proposal.patternId !== pattern.id) throw new Error("Proposal does not match its evidence pattern");
