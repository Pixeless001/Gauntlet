...
export async function resolvePackageKnowledge(cwd: string, name: string, query: string, providers: KnowledgeProviders = {}): Promise<KnowledgeFact | null> {
  ...
  for (const path of ["README.md", "docs/index.md"]) try { const content = await readFile(resolve(root, path), "utf8"); if (matches(content, query)) return { package: name, version, query, source: "local_docs", proof: `${name}/${path}` }; } catch { /* unavailable local documentation */ }
