...
export async function resolvePackageKnowledge(cwd: string, name: string, query: string, providers: KnowledgeProviders = {}): Promise<KnowledgeFact | null> {
  ...
  for (const candidate of entrypoints(metadata)) {
    ...
    try { const size = (await stat(path)).size; if (size > 256_000) continue; const content = await readFile(path, "utf8"); if (matches(content, query)) return { package: name, version, query, source: candidate.source, evidence: `${name}/${candidate.path}` }; } catch { /* unavailable local evidence */ }
  }
  if (metadata && matches(Object.keys(metadata).join(" "), query)) return { package: name, version, query, source: "package_metadata", evidence: `${name}/package.json` };
