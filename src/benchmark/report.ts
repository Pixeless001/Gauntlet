...
export interface ArmSummary { arm: "baseline" | "treatment"; runs: number; acceptanceRate: number; preservationRate: number; slopFreeRate: number; firstCleanPassRate: number; medianWallMs: number; medianLocAdded: number; tokensReported: number }
...
export interface BenchmarkReport {
  ...
  tasks: { task: string; repeats: number; acceptance: { baseline: number; treatment: number }; preservation: { baseline: number; treatment: number }; slopFree: { baseline: number; treatment: number }; slopCodes: { baseline: string[]; treatment: string[] }; medianWallMs: { baseline: number; treatment: number }; medianLocAdded: { baseline: number; treatment: number } }[];
  outcomes: ArmOutcome[];
}

export function buildReport(outcomes: ArmOutcome[], meta: { harness: string; model?: string | undefined; endpoint?: string | undefined }): BenchmarkReport {
  const arms = ["baseline", "treatment"] as const;
  const tasks = [...new Set(outcomes.map((outcome) => outcome.task))].sort().map((task) => {
    const forTask = (arm: "baseline" | "treatment") => outcomes.filter((outcome) => outcome.task === task && outcome.arm === arm);
    const rates = (arm: "baseline" | "treatment", key: "acceptance" | "preservation" | "slopFree") => rate(forTask(arm).map((outcome) => outcome[key]));
    return {
      task,
  ...
  const summarize = (arm: "baseline" | "treatment"): ArmSummary => {
...
export function formatReport(report: BenchmarkReport): string {
  ...
  for (const task of report.tasks) {
    ...
    if (task.slopCodes.treatment.length) lines.push(`  treatment slop: ${[...new Set(task.slopCodes.treatment)].join(", ")}`);
    if (task.slopCodes.baseline.length) lines.push(`  baseline slop: ${[...new Set(task.slopCodes.baseline)].join(", ")}`);
  }
  return lines.join("\n");
}

export async function saveReport(cwd: string, report: BenchmarkReport): Promise<string> {
...
function rate(flags: boolean[]): number { return flags.length ? flags.filter(Boolean).length / flags.length : 0; }
function codes(outcomes: ArmOutcome[]): string[] { return outcomes.flatMap((outcome) => outcome.slopFindings.map((finding) => finding.code)); }
function median(values: number[]): number { if (!values.length) return 0; const sorted = [...values].sort((a, b) => a - b); return sorted[Math.floor((sorted.length - 1) / 2)] ?? 0; }
