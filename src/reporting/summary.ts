...
export function formatSummary(value: TaskMeasurement, color = process.stdout.isTTY): string {
  ...
  return [pc.bold("GAUNTLET"), paint(value.completion !== "incomplete", value.completion === "incomplete" ? "✗ INCOMPLETE" : "✓ COMPLETE"), paint(value.clean, `${mark(value.clean)} CLEAN`), paint(value.verified, `${mark(value.verified)} VERIFIED`), paint(value.firstPass, `${mark(value.firstPass)} FIRST PASS`), "", `Time       ${seconds}s`, `Attempts   ${value.attempts}`, `Files      ${value.files}`, `LoC        +${value.added}/-${value.removed}`, `Checks     ${value.checksRun}`, ...(findings.length ? ["", "Findings:", ...findings] : []), ...(failures.length ? ["", "Failed proof:", ...failures] : [])].join("\n");
}
