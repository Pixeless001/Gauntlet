---
name: verify
description: Resolve meaningful change uncertainty with relevant proof.
triggers: before-stop
budget: cheapest sufficient proof
---

# VERIFY

1. Start from acceptance behavior and behavior that must remain unchanged.
2. Use cheap repository-native proof first: types, impacted tests, test integrity, and relevant static checks.
...
7. Treat every check as proof rather than truth.

Stop when remaining uncertainty is acceptably low and another check is unlikely to change the acceptance decision. Report concrete proof, not a confidence score.
